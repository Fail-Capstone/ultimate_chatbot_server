...
router.put("/addpatterns", verifyToken, async (req, res) => {
    const { tag, patterns } = req.body;

    if (!tag || patterns.length === 0) {
        return res
            .status(400)
    ...
    try {
        let updatedPatterns = await Intent.find({ tag });
        updatedPatterns = { ...updatedPatterns[0], patterns };
